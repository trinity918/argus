...
A streaming system that ingests live exchange order-book and trade feeds, detects
market-manipulation **footprints** (spoofing, layering, momentum ignition, quote
stuffing, wash trading) in real time, and writes every alert to a **tamper-evident,
...
| Low-latency infrastructure | In-memory streaming detectors; **p99 detection latency measured in microseconds** |
| Massively scalable | Stateless-per-symbol detection; ingestion and detection scale independently over NATS |
...
```
                          ┌───────────────┐
   Binance WS/REST  ──────▶│   Ingestor    │  normalize • local-book sync (U/u) • backpressure
   (or synthetic replay)  │     (Go)      │  reconnect • sequence-gap resync
                          └──────┬────────┘
...
internal/
  ...
  exchange/binance/  WS client + book-sync state machine
...
The Go image is a single static binary per service. To deploy the dashboard/API:

```bash
gcloud run deploy argus-api --source . --port 8080 \
  --set-env-vars NATS_URL=nats://<your-nats>:4222
```

Run NATS (or a managed alternative) reachable from the services, and mount/share a
volume for the audit trail between the detector and API. For a single-service
deployment, `argusd` runs the whole stack in one container.
